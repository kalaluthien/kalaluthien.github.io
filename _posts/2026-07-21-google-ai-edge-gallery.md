...
> Research Google AI Edge Gallery codebases, features, and models.
...
- **Features**: the app ships **AI Chat, Ask Image, Audio Scribe, Prompt Lab, Agent Skills, Mobile Actions, and Tiny Garden** — not the "Ask Image / Prompt Lab / AI Chat / function calling / RAG / audio" set assumed going in. "Agent Skills" is the function-calling feature, and MCP (`io.modelcontextprotocol:kotlin-sdk`) is one of **four** tool backends behind `tools/ToolDispatcher.kt` — alongside Android intents (`RunIntentTool`), JavaScript skills in a WebView (`RunJsTool`), and `LoadSkillTool`. **There is no RAG feature** — no embedding model, no vector store anywhere in the tree — even though the `.litertlm` format itself reserves a section for one.
- **Models**: 9 models in the live allowlist, all `litert-community/` or `google/` Hugging Face repos: Gemma 4 E2B/E4B, Gemma 3n E2B/E4B, Gemma3-1B, Qwen2.5-1.5B-Instruct, DeepSeek-R1-Distill-Qwen-1.5B, and two FunctionGemma-270M fine-tunes. Quantization is at best inferable from filename suffixes (`int4`, `q8`) or description prose ("with 4-bit quantization") — and for the two Gemma 4 entries it is stated nowhere at all, the filename being a bare `gemma-4-E2B-it.litertlm`. **No entry states per-channel vs. per-tensor granularity anywhere.** **Hammer is not a Gallery model** — it is a third-party (MadeAgents) family that shows up only as a conversion target in Google's separate `litert-torch` tooling repo.
- **Performance**: Google's own docs table (`developers.google.com/edge/litert-lm/overview`, v0.14.0, last updated 2026-07-09) reports prefill/decode/TTFT/memory for Gemma-4-E2B across 6 platforms × CPU/GPU. **The oft-quoted "52 tok/s" is one row of eleven, not a ceiling** — a MacBook Pro M4 Max hits 160 tok/s decode on GPU, an RTX 4090 hits 143 tok/s, a Raspberry Pi 5 floors at 8 tok/s, and Google's own speculative-decoding numbers take the same S26 Ultra to 91.7 tok/s. **That table has no NPU row**, but one Gemma-4 NPU figure does exist off it: Qualcomm Dragonwing IQ8, 3,747 prefill / 31.7 decode, on the E2B model card's IoT table — embedded silicon, not a phone. The app does self-report benchmark stats, through a real harness (`BenchmarkViewModel.runBenchmark()` calling LiteRT-LM's own `benchmark()` function), not a marketing claim.
- **Where it breaks**: RAM gating exists (`minDeviceMemoryInGb` per model) but is a **dismissable soft warning**, never a hard block — the README states no RAM number at all. NPU support is limited at **runtime, not compile time**: on Android the NPU executor ships by default (`LITERT_DISABLE_NPU` is selected only for `@platforms//os:ios`), and the real gate is `dlopen` of a per-vendor dispatch `.so` that is absent on most devices — falling back to a hard error, not to a stub. The issue tracker documents concrete failures across MediaTek (unsupported, crashes), Samsung Exynos (`INTERNAL` engine-creation error), Intel VPU generation mismatches, and even Google's own Tensor G5 (DMA-buf exhaustion on the base Pixel 10). Licensing is gated dynamically — the app tries an unauthenticated download first and falls back to Hugging Face OAuth on **any non-200**, not specifically a 403 — plus a separate Gemma-terms-of-use gate that both excludes the Gemma 4 models *and* is currently inert for every allowlisted model. Mid-range and older hardware fails in varied, non-uniform ways: EMUI crash-on-launch, a Gemma-4-E4B regression on a Pixel 6a under an Android 17 beta (cause unconfirmed), KV-cache-growth OOM in long chats, and raw `SIGILL` on CPUs lacking NEON/SVE.
...
4. The full multi-platform benchmark table (not just the one headline number), the absence of an NPU row in it, and the single off-table Gemma-4 NPU figure that does exist on embedded silicon.
...
Vendor NPU silicon is bound at **runtime**, not compile time, through `dlopen`: `litert::SharedLibrary::Load(shared_lib_path, ...)`, resolving the symbol `"LiteRtDispatchGetApi"` (`LiteRT:litert/runtime/dispatch/litert_dispatch.cc#158-176`). This is corroborated by a device log quoted in a real bug report: *"Loading shared library: /data/local/tmp/gemma/libLiteRtDispatch_Qualcomm.so"* (LiteRT-LM issue #1979). When no vendor library is found the loader logs *"No dispatch library found in %s"* and fails hard; it does **not** fall through to a stub. A pure stub does exist — all ~35 API entry points returning `kLiteRtStatusErrorUnsupported` (`LiteRT:litert/runtime/dispatch/litert_dispatch_dummy.cc#22-260`) — but a code search shows `litert_dispatch_dummy` is referenced only by its own `BUILD` file, so nothing depends on it and it is not the default path. Real vendor implementations live under `litert/vendors/{broadcom,google_tensor,intel_openvino,mediatek,qualcomm,samsung}`; Qualcomm's is the most built-out, with its own QNN graph-legalization compiler plugin.
...
**Per-channel vs. per-tensor quantization granularity is absent from every entry across all 13 historical allowlist versions checked** (`1_0_4` through `1_0_15`, plus an `ios_1_0_0` variant) — quantization scheme is never stated as a structured field, only inferable from the filename suffix (`int4`, `q8`) or, for some entries, from description prose ("with 4-bit quantization"). For the two Gemma 4 entries it is not inferable at all — bare `gemma-4-E2B-it.litertlm`, and a description mentioning only multi-modality and 32K context. Do not treat any granularity claim about these specific artifacts as sourced.
...
**The widely-quoted "52 tok/s, Galaxy S26 Ultra, GPU" figure is one row of eleven in this table, not the runtime's ceiling.** A MacBook Pro M4 Max decodes Gemma-4-E2B over 3× faster on GPU (160 tok/s), and an RTX 4090 hits 143 tok/s; the floor is a Raspberry Pi 5 on CPU at 8 tok/s with a 7.8-second TTFT. **Every row in this table is CPU or GPU — Google's featured benchmark table carries no NPU row for Gemma 4** (one does exist off-table, on embedded silicon — see below). A second table on the same page adds Gemma-4-E4B and older models (Gemma-3n, Gemma3-1B, FunctionGemma, phi-4-mini, Qwen2.5-1.5B/0.5B, Qwen3-0.6B) across additional devices (Samsung S24/S25 Ultra, Vivo X300 Pro), same CPU/GPU-only pattern.
...
### NPU numbers: one Gemma-4 row exists, on embedded silicon

The Gemma-4 NPU picture is thin, not empty. The
`litert-community/gemma-4-E2B-it-litert-lm` model card carries exactly one NPU
row, in its **IoT** section:

| Device | Backend | Prefill | Decode | TTFT | Model size (MB) | CPU mem (MB) |
| --- | --- | --- | --- | --- | --- | --- |
| Qualcomm Dragonwing IQ8 (IQ-8275) | NPU | 3,747 | 31.7 | 0.3 | 2,967 | 1,869 |

Dragonwing IQ8 is embedded/industrial silicon, not a handset, and this is a
separate NPU-converted artifact — 2,967 MB against 2,583 MB for the CPU and GPU
builds — benchmarked at 4,096 context. **The defensible claim is therefore that
no Gemma-4 NPU figure exists for a phone, not that none exists at all**, and
that Google's own featured benchmark table carries no NPU row.

Older Gemma 3n NPU numbers also exist, on Hugging Face model cards:
...
**Explicit gaps**: no NPU figure exists for Gemma-4-E2B on a *phone*, and none at all for Gemma-4-E4B — the single Gemma-4 NPU row above is embedded silicon (Dragonwing IQ8). Google's docs table is also not the complete published picture: the Hugging Face model card is a superset, adding Jetson Orin Nano (CPU 109/12.2, GPU 1,142/24.2) and WebGPU on an M4 Max (4,853/73), plus a separate speculative-decoding table where the S26 Ultra GPU reaches **91.7 tok/s decode** against a 51.5 baseline — Google's own numbers refuting any reading of 52 tok/s as that phone's ceiling. Neither the Gallery nor the LiteRT README contains its own benchmark table — both defer entirely to the external docs page. No GitHub issue or discussion was found with an extractable numeric tok/s or TTFT figure (one issue, #727, has benchmark results as a screenshot image only — not machine-extractable text, and not quoted here as a number for that reason).
